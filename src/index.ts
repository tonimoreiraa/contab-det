...
async function runCompany(page: Page, company: any, i: number) {
  if (i != 0) {
  ...
  const procuradorButton = await page.waitForSelector(
  ...
  await page.click(
    "body > modal-container > div > div > app-modal-perfil > div.modal-body > form > div.row.barra-botoes > div > button.br-button.is-primary"
  );
  ...
  if (
    !(
      (await page.$(".br-message.is-danger")) ||
      (await page.$(".br-input.is-invalid"))
    )
  ) {
    logger.info(`Logado na empresa ${company["EMPRESA"]}: ${cnpj}`);

    await page.goto("https://det.sit.trabalho.gov.br/cadastro", {
      waitUntil: "networkidle0",
    });

    const hasContactRegistered = !(await page.$(
      "#content > app-cadastro > br-title > br-alert-messages > aside > div.br-message.is-warning"
    ));
    logger.info(
      `Tem contatos registrados: ${hasContactRegistered ? "Sim" : "Não"}`
    );

    if (!hasContactRegistered) {
      logger.info("Registrando palavra chave.");

      const keyworkInput = await page.waitForSelector(
        "#content > app-cadastro > form > div:nth-child(1) > div > br-input > div > div > input"
      );
      await keyworkInput?.type("CLEOSELMA");

      await page.click(
        "#content > app-cadastro > form > div.br-table > div > span > a"
      );

      logger.info("Registrando contato.");
      const nameInput = await page.waitForSelector(
        "body > modal-container > div > div > app-contato-cadastro > div.modal-body > form > div > div:nth-child(1) > br-input > div > div > input"
      );
      const emailInput = await page.$(
        "body > modal-container > div > div > app-contato-cadastro > div.modal-body > form > div > div:nth-child(2) > br-input > div > div > input"
      );
      const phoneInput = await page.$(
        "body > modal-container > div > div > app-contato-cadastro > div.modal-body > form > div > div:nth-child(3) > br-input > div > div > input"
      );
      ...
      await page.click(
        "body > modal-container > div > div > app-contato-cadastro > div.modal-footer > button.br-button.is-primary"
      );
      await page.waitForFunction(
        () => !document.body.classList.contains("modal-open")
      );
      ...
      await page.click(
        "#content > app-cadastro > form > div.row.barra-botoes > div > button"
      );
      await page.waitForSelector(
        "#content > app-cadastro > br-title > br-alert-messages > aside > div"
      );
...
async function main() {
  ...
  while (i < companies.length) {
    try {
      ...
      await runCompany(page, company, i)
        .catch(_ => {
          i--
        })
        .then(() => {
